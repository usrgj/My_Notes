...
modified: '2025-10-26T07:33:54.176Z'
---
...
<style>
  ...
  body {
    ...
    .preview {
      ...
      padding: 0 0 0 5%; /* 添加左内边距 */
      background-color: rgb(200, 200, 15);
      border-color: rgb(230, 229, 229);
      text-align: left; /* 确保文本左对齐 */
      width: auto; /* 自动宽度，不拉伸 */
      margin-right: auto; /* 确保内容不会居中 */
    }
    ...
    .theme-light .preview {
      ...
      background-color: FDF6E3;
      border-color: rgb(230, 229, 229);
      color: rgb(31, 31, 31) flex;
      justify-content: flex-start; /* 改为flex-start而不是center */
      align-items: flex-start; /* 改为flex-start而不是center */

    }

    .theme-light .preview table {
      display: table;
      place-items: center;
    ...
    .theme-light .preview hr {
      background-color: rgba(62, 215, 146, 0.451)
    }
...
[Official Document](https://docs.b-human.de/coderelease2025/getting-started/initial-setup/#setting-up-the-development-computer)
> 2025官方仓库代码更新后一周内修改。由于官方已经使用ubuntu24，笔者在ubuntu22且混乱的环境中测试，有许多没法顾忌的地方。本次修改移除了所有windows相关的内容，添加为ubuntu22环境配置等内容。
...
> 机器人操作系统（robot operating system, ros）与ubuntu有着密切的关联，因此ubuntu是最佳选择。此外，ubuntu没有想象中的那么难用，可以试着将其作为主系统使用
...
### 1镜像包
[下载，记得选桌面版](https://cn.ubuntu.com/download/alternative-downloads)，或找人拷贝,选择22或24都行
### 2制作启动盘
找人拷贝制作启动盘的软件
![](./use/1-1.jpg)
***下面选择语言时建议选英文***
...
> 这些基本的包是往下进行的基础  
#### 安装基础包
[文档对应**Linux**](https://docs.b-human.de/coderelease2025/getting-started/initial-setup/#setting-up-the-development-computer)
在任意终端运行以下命令  
```bash
# ubuntu22运行
sudo apt update
sudo apt install -y build-essential tar
sudo apt install ccache clang cmake exfatprogs git graphviz libasound2-dev libbox2d-dev libgl-dev  llvm mold net-tools ninja-build pigz qt6-base-dev rsync xxd

sudo add-apt-repository ppa:okirby/qt6-backports  # 推荐PPA，提供Qt6.4+向后移植版本
sudo apt update
sudo apt install libqt6svg6-dev  # 安装SVG开发库

# ubuntu24 运行
sudo apt update
sudo apt install -y build-essential tar
sudo apt install ccache clang cmake exfatprogs git graphviz libasound2-dev libbox2d-dev libgl-dev libstdc++-13-dev llvm mold net-tools ninja-build pigz qt6-base-dev qt6-svg-dev rsync xxd
```
...
## 6 第一次刷机
> 机器人内原本有设置好的内容，这里是要将其重置,并刷入操作系统。你需要一个至少2G的U盘。顺便一提，学院发的U盘能刷第二次，不能刷第一次；同样的，其它U盘也可能有类似的问题……

如果是正常使用的U盘，比如说能正常存东西的U盘，插入U盘后，在dock将U盘卸载（不能在文件夹中卸载）
![](./use/6-1.gif)

在`/path/to/your/BHumanCodeRelease`打开终端
```bash
# 插入U盘后
ls /dev/sd*
# device
# sd表示SCSI Disk， sd后跟的字母abdc表示U盘插入顺序，数字表示分区号
# 记住你的U盘的设备，比如说/dev/sda(如果有/dev/sda1,记住你的设备仍然是/dev/sda)

# 将文件刷入U盘
sudo ./Install/flash <source image> <target device>

# 例如
sudo ./Install/flash /home/gj/Desktop/soc-res/nao-2.8.5.11_ROBOCUP_ONLY_with_root.opn /dev/sda
```
